...
public class SubProcessResource {
    ...
    public List<TaskModel> getTasks_subProcess(@PathParam("id") String id, @QueryParam("user") final String user, @QueryParam("group") final List<String> groups) {
        return processService.getTasks(process, id, user, groups).orElseThrow(NotFoundException::new).stream().map(org.acme.travels.SubProcess_TaskModelFactory::from).collect(Collectors.toList());
    }
}
